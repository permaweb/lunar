...
// Announced peers are addressed as `ip:port` and serve plain HTTP; a bare hostname is a gateway that serves HTTPS.
const HTTP_HOST = /^(\[|localhost([:/]|$)|(\d{1,3}\.){3}\d{1,3}([:/]|$)|[^/]+:\d)/;
...
export function normalizeArweaveNode(value: string): string | null {
	try {
		...
		const url = new URL(input.includes('://') ? input : `${HTTP_HOST.test(input) ? 'http' : 'https'}://${input}`);
