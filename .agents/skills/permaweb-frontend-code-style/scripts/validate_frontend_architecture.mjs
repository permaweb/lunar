...
const projectRoot = path.resolve(
	rootArgumentIndex >= 0 ? argumentsList[rootArgumentIndex + 1] : process.cwd(),
);
...
function checkStandardDirectory(directory) {
	...
	if (fs.existsSync(barrelPath)) {
		...
		const namedDefaultExport = new RegExp(
			`export\\s*{\\s*default\\s+as\\s+${escapedName}\\s*}\\s*from\\s*['"]\\./${escapedName}['"]`,
		);
...
for (const featureDirectory of directories(path.join(sourceRoot, 'features'))) {
	...
	if (fs.existsSync(featureAtoms)) {
		report(
			'feature-atom',
			featureAtoms,
			'Feature UI must compose shared root atoms instead of owning primitives.',
		);
	}
...
for (const filePath of sourceFiles.filter((candidate) => /\.(?:jsx|tsx)$/.test(candidate))) {
	...
	if (navigationNamePattern.test(componentName) && !isNavigationFile) {
		report(
			'navigation-ownership',
			filePath,
			`${componentName} must live under ${sourceRootName}/navigation/.`,
		);
	}
}
...
for (const [filePath, sourceText] of sourceTexts) {
	...
	if (
		/\bReact\./.test(sourceText) &&
		!reactImports.some((reactImport) => reactImport[1].trim() === 'React')
	) {
		report('react-default-import', filePath, "React namespace usage requires `import React from 'react';`.");
	}
	if (
		/^\s*handle[A-Z]\w*\??\s*(?::\s*(?:\([^;]*?\)\s*=>|[\w.<>]+Handler\b)|\([^)]*\)\s*:)/gm.test(sourceText)
	) {
		report(
			'callback-prop-name',
			filePath,
	...
	const executableSourceText = sourceText
		.replace(/\/\*[\s\S]*?\*\//g, '')
		.replace(/^\s*\/\/.*$/gm, '')
		.replace(/>[^<]*</gs, '><');
...
for (const megaBarrel of [
	...
	if (fs.existsSync(megaBarrel))
		report(
			'mega-barrel',
			megaBarrel,
			'Remove root mega barrels; use component or feature public boundaries.',
		);
}
...
function parseImports(sourceText) {
	...
	for (const match of sourceText.matchAll(
		/^\s*(import|export)\s+([^;]*?)\s+from\s+['"]([^'"]+)['"]\s*;?/gms,
	)) {
	...
	for (const budget of directoryBudgets) {
		...
		if (!outputFiles.length || newestModification(outputFiles) < newestInput) {
			report(
				'performance-freshness',
				outputPath,
				'Production output is older than a configured build input.',
			);
		}
